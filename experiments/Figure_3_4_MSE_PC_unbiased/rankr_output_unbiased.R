...
for (seed in 1:200){
    ...
    if (file.exists(filename)) {
        load(filename)
            tau_hat_vars = c(tau_hat_vars,var(tau_hats))
            tau_hat_means = c(tau_hat_means,mean(tau_hats))
        cases = c(cases, case)
        ns = c(ns, n)
        rhons = c(rhons, rhon)
    }
...
dat1 = data.frame(tau_hat_vars = tau_hat_vars, tau_hat_means = tau_hat_means,
...
for (rate_of_converge in 1:2){
    dat2 = data.frame(vars = tau_hat_vars, means = tau_hat_means,
                      Setting = as.factor(cases), ns = ns, rhons = rhons,
                      biases = tau_hat_means - tau_IND_cases[cases],
    ...
    #title = sprintf('Unbiased Estimator: $\\log_{10}( MSE(\\hat{\\tau}))$ when $\\rho_n = n^{-%i/5}$', rate_of_converge)
    #p = p + ggtitle(TeX(title))
    p = p + xlab(TeX('$\\log_{10}(n)$'))
    p = p + ylab(TeX('$\\log_{10}( MSE $)$'))
    subtitle = sprintf('Slope of the reference lines  $= %i/5$', 5-2*rate_of_converge)
    p = p + labs(caption = TeX(subtitle))
    for (i in (-10:10)/5){
        p = p + geom_abline(intercept = i, slope = 1-2*rate_of_converge/5, color = "grey", linetype = 3)
    }
    print(p)
}
